    ...
    class NonRegularHierarchicNetworkContainer : AbstractNetworkContainer
    {
        public override Int32 Size
        {
            get
            {
                ...
                {
                    int vertexCount = 0;
                    for (int i = 0; i < Branches[level - 1].Count; ++i)
                    {
        ...
        private Int32 edgesCount = -1;
        public override Int32 EdgesCount
        {
            get
            {
                if (edgesCount == -1)
                    edgesCount = CalculateNumberOfEdges(0, 0);
                return edgesCount;
            }
        }
        ...
        private int level = 0;
        private List<List<int>> branches;
        ...
        public void SetBranches(List<List<int>> branches)
        {
            // Fill this.branches from branch
            this.branches = new List<List<int>>(new List<int>[branches.Count]);
            for (int i = 0; i < branches.Count; ++i)
            {
                this.branches[i] = new List<int>(new int[branches[i].Count]);
                for (int j = 0; j < this.branches[i].Count; ++j)
                    this.branches[i][j] = this.branches[i][j];
            }

            this.level = branches.Count;
        }
        ...
        public BitArray TreeNode(int currentLevel, long currentNodeNumber)
        {
            ...
            // TODO long to int convertion
            int branchesCount = branches[currentLevel][(int)currentNodeNumber];
        ...
        public int[,] NodeMatrix(int currentLevel, long currentNodeNumber)
        {
            ...
            // TODO long to int convertion
            int branchSize = branches[currentLevel][(int)currentNodeNumber];
        ...
        public int TreeIndexStep(int v, int currentLevel)
        {
            ...
            int counter = 0;
            bool found = false;
            for (int i = 0; i < branches[currentLevel].Count; ++i)
            {
