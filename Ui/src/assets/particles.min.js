var pJS = function (e, a) {
    var t = document.querySelector("#" + e + " > .particles-js-canvas-el"), i = 0;
    this.pJS = {
        fps_limit: 60,
    ...
    a && Object.deepExtend(s, a), s.tmp.obj = {
        ...
        s.canvas.el.width = s.canvas.w, s.canvas.el.height = s.canvas.h, s && s.interactivity.events.resize && window.addEventListener("resize", function () {
            s.canvas.w = s.canvas.el.offsetWidth, s.canvas.h = s.canvas.el.offsetHeight, s.tmp.retina && (s.canvas.w *= s.canvas.pxratio, s.canvas.h *= s.canvas.pxratio), s.canvas.el.width = s.canvas.w, s.canvas.el.height = s.canvas.h, s.particles.move.enable || (s.fn.particlesEmpty(), s.fn.particlesCreate(), s.fn.particlesDraw(0), s.fn.vendors.densityAutoParticles()), s.fn.vendors.densityAutoParticles()
        })
    ...
    }, s.fn.particle = function (e, a, t) {
        if (this.radius = (s.particles.size.random ? Math.random() : 1) * s.particles.size.value, s.particles.size.anim.enable && (this.size_status = !1, this.vs = s.particles.size.anim.speed / 100, s.particles.size.anim.sync || (this.vs = this.vs * Math.random())), this.x = t ? t.x : Math.random() * s.canvas.w, this.y = t ? t.y : Math.random() * s.canvas.h, this.x > s.canvas.w - 2 * this.radius ? this.x = this.x - this.radius : this.x < 2 * this.radius && (this.x = this.x + this.radius), this.y > s.canvas.h - 2 * this.radius ? this.y = this.y - this.radius : this.y < 2 * this.radius && (this.y = this.y + this.radius), s.particles.move.bounce && s.fn.vendors.checkOverlap(this, t), this.color = {}, "object" == typeof e.value) if (e.value instanceof Array) {
        ...
        var e = this;
        if (void 0 != e.radius_bubble) var a = e.radius_bubble; else a = e.radius;
        if (void 0 != e.opacity_bubble) var t = e.opacity_bubble; else t = e.opacity;
        if (e.color.rgb) var i = "rgba(" + e.color.rgb.r + "," + e.color.rgb.g + "," + e.color.rgb.b + "," + t + ")"; else i = "hsla(" + e.color.hsl.h + "," + e.color.hsl.s + "%," + e.color.hsl.l + "%," + t + ")";
        switch (s.canvas.ctx.fillStyle = i, s.canvas.ctx.beginPath(), e.shape) {
            case"circle":
                s.canvas.ctx.arc(e.x, e.y, a, 0, 2 * Math.PI, !1);
                break;
            case"edge":
                s.canvas.ctx.rect(e.x - a, e.y - a, 2 * a, 2 * a);
                break;
            case"triangle":
                s.fn.vendors.drawShape(s.canvas.ctx, e.x - a, e.y + a / 1.66, 2 * a, 3, 2);
                break;
            case"polygon":
                s.fn.vendors.drawShape(s.canvas.ctx, e.x - a / (s.particles.shape.polygon.nb_sides / 3.5), e.y - a / .76, 2.66 * a / (s.particles.shape.polygon.nb_sides / 3), s.particles.shape.polygon.nb_sides, 1);
                break;
            case"star":
                s.fn.vendors.drawShape(s.canvas.ctx, e.x - 2 * a / (s.particles.shape.polygon.nb_sides / 4), e.y - a / 1.52, 2 * a * 2.66 / (s.particles.shape.polygon.nb_sides / 3), s.particles.shape.polygon.nb_sides, 2);
                break;
            case"image":
                ;
                if ("svg" == s.tmp.img_type) var n = e.img.obj; else n = s.tmp.img_obj;
                n && s.canvas.ctx.drawImage(n, e.x - a, e.y - a, 2 * a, 2 * a / e.img.ratio)
        }
    ...
    }, s.fn.particlesUpdate = function (e) {
        for (var a = 0; a < s.particles.array.length; a++) {
            var t = s.particles.array[a];
            if (s.particles.move.enable) {
                var i = s.particles.move.speed / 10;
                t.x += t.vx * i * e, t.y += t.vy * i * e
            }
            if (s.particles.opacity.anim.enable && (1 == t.opacity_status ? (t.opacity >= s.particles.opacity.value && (t.opacity_status = !1), t.opacity += t.vo) : (t.opacity <= s.particles.opacity.anim.opacity_min && (t.opacity_status = !0), t.opacity -= t.vo), t.opacity < 0 && (t.opacity = 0)), s.particles.size.anim.enable && (1 == t.size_status ? (t.radius >= s.particles.size.value && (t.size_status = !1), t.radius += t.vs) : (t.radius <= s.particles.size.anim.size_min && (t.size_status = !0), t.radius -= t.vs), t.radius < 0 && (t.radius = 0)), "bounce" == s.particles.move.out_mode) var n = {
                x_left: t.radius,
                x_right: s.canvas.w,
                y_top: t.radius,
                y_bottom: s.canvas.h
            ...
            switch (t.x - t.radius > s.canvas.w ? (t.x = n.x_left, t.y = Math.random() * s.canvas.h) : t.x + t.radius < 0 && (t.x = n.x_right, t.y = Math.random() * s.canvas.h), t.y - t.radius > s.canvas.h ? (t.y = n.y_top, t.x = Math.random() * s.canvas.w) : t.y + t.radius < 0 && (t.y = n.y_bottom, t.x = Math.random() * s.canvas.w), s.particles.move.out_mode) {
                case"bounce":
                    t.x + t.radius > s.canvas.w ? t.vx = -t.vx : t.x - t.radius < 0 && (t.vx = -t.vx), t.y + t.radius > s.canvas.h ? t.vy = -t.vy : t.y - t.radius < 0 && (t.vy = -t.vy)
            }
            if (isInArray("grab", s.interactivity.events.onhover.mode) && s.fn.modes.grabParticle(t), (isInArray("bubble", s.interactivity.events.onhover.mode) || isInArray("bubble", s.interactivity.events.onclick.mode)) && s.fn.modes.bubbleParticle(t), (isInArray("repulse", s.interactivity.events.onhover.mode) || isInArray("repulse", s.interactivity.events.onclick.mode)) && s.fn.modes.repulseParticle(t), s.particles.line_linked.enable || s.particles.move.attract.enable) for (var r = a + 1; r < s.particles.array.length; r++) {
                var c = s.particles.array[r];
                s.particles.line_linked.enable && s.fn.interact.linkParticles(t, c), s.particles.move.attract.enable && s.fn.interact.attractParticles(t, c), s.particles.move.bounce && s.fn.interact.bounceParticles(t, c)
            }
        }
    }, s.fn.particlesDraw = function (e) {
        s.canvas.ctx.clearRect(0, 0, s.canvas.w, s.canvas.h), s.fn.particlesUpdate(e);
        for (var a = 0; a < s.particles.array.length; a++) {
            s.particles.array[a].draw()
        }
    ...
    }, s.fn.interact.linkParticles = function (e, a) {
        var t = e.x - a.x, i = e.y - a.y, n = Math.sqrt(t * t + i * i);
        if (n <= s.particles.line_linked.distance) {
            ...
            if (r > 0) {
                ...
                s.canvas.ctx.strokeStyle = "rgba(" + c.r + "," + c.g + "," + c.b + "," + r + ")", s.canvas.ctx.lineWidth = s.particles.line_linked.width, s.canvas.ctx.beginPath(), s.canvas.ctx.moveTo(e.x, e.y), s.canvas.ctx.lineTo(a.x, a.y), s.canvas.ctx.stroke(), s.canvas.ctx.closePath()
            }
        }
    }, s.fn.interact.attractParticles = function (e, a) {
        var t = e.x - a.x, i = e.y - a.y;
        if (Math.sqrt(t * t + i * i) <= s.particles.line_linked.distance) {
            var n = t / (1e3 * s.particles.move.attract.rotateX), r = i / (1e3 * s.particles.move.attract.rotateY);
            e.vx -= n, e.vy -= r, a.vx += n, a.vy += r
        }
    }, s.fn.interact.bounceParticles = function (e, a) {
        var t = e.x - a.x, i = e.y - a.y;
        Math.sqrt(t * t + i * i) <= e.radius + a.radius && (e.vx = -e.vx, e.vy = -e.vy, a.vx = -a.vx, a.vy = -a.vy)
    }, s.fn.modes.pushParticles = function (e, a) {
        s.tmp.pushing = !0;
        for (var t = 0; t < e; t++) s.particles.array.push(new s.fn.particle(s.particles.color, s.particles.opacity.value, {
            x: a ? a.pos_x : Math.random() * s.canvas.w,
            y: a ? a.pos_y : Math.random() * s.canvas.h
        })), t == e - 1 && (s.particles.move.enable || s.fn.particlesDraw(0), s.tmp.pushing = !1)
    }, s.fn.modes.removeParticles = function (e) {
        s.particles.array.splice(0, e), s.particles.move.enable || s.fn.particlesDraw(0)
        ...
        if (s.interactivity.events.onhover.enable && isInArray("bubble", s.interactivity.events.onhover.mode)) {
            var a = e.x - s.interactivity.mouse.pos_x, t = e.y - s.interactivity.mouse.pos_y,
                i = 1 - (l = Math.sqrt(a * a + t * t)) / s.interactivity.modes.bubble.distance;
            ...
            if (l <= s.interactivity.modes.bubble.distance) {
                if (i >= 0 && "mousemove" == s.interactivity.status) {
                    if (s.interactivity.modes.bubble.size != s.particles.size.value) if (s.interactivity.modes.bubble.size > s.particles.size.value) {
                        (c = e.radius + s.interactivity.modes.bubble.size * i) >= 0 && (e.radius_bubble = c)
                    } else {
                        var r = e.radius - s.interactivity.modes.bubble.size, c = e.radius - r * i;
                        e.radius_bubble = c > 0 ? c : 0
                    }
                    var o;
                    if (s.interactivity.modes.bubble.opacity != s.particles.opacity.value) if (s.interactivity.modes.bubble.opacity > s.particles.opacity.value) (o = s.interactivity.modes.bubble.opacity * i) > e.opacity && o <= s.interactivity.modes.bubble.opacity && (e.opacity_bubble = o); else (o = e.opacity - (s.particles.opacity.value - s.interactivity.modes.bubble.opacity) * i) < e.opacity && o >= s.interactivity.modes.bubble.opacity && (e.opacity_bubble = o)
                }
            } else n();
            "mouseleave" == s.interactivity.status && n()
            ...
            if (s.tmp.bubble_clicking) {
                a = e.x - s.interactivity.mouse.click_pos_x, t = e.y - s.interactivity.mouse.click_pos_y;
                var l = Math.sqrt(a * a + t * t), v = ((new Date).getTime() - s.interactivity.mouse.click_time) / 1e3;
                v > s.interactivity.modes.bubble.duration && (s.tmp.bubble_duration_end = !0), v > 2 * s.interactivity.modes.bubble.duration && (s.tmp.bubble_clicking = !1, s.tmp.bubble_duration_end = !1)
            }

            function p(a, t, i, n, r) {
                if (a != t) if (s.tmp.bubble_duration_end) void 0 != i && (o = a + (a - (n - v * (n - a) / s.interactivity.modes.bubble.duration)), "size" == r && (e.radius_bubble = o), "opacity" == r && (e.opacity_bubble = o)); else if (l <= s.interactivity.modes.bubble.distance) {
                    if (void 0 != i) var c = i; else c = n;
                    if (c != a) {
                        var o = n - v * (n - a) / s.interactivity.modes.bubble.duration;
        ...
        if (s.interactivity.events.onhover.enable && isInArray("repulse", s.interactivity.events.onhover.mode) && "mousemove" == s.interactivity.status) {
            var a = e.x - s.interactivity.mouse.pos_x, t = e.y - s.interactivity.mouse.pos_y,
                i = Math.sqrt(a * a + t * t), n = a / i, r = t / i,
                c = clamp(1 / (l = s.interactivity.modes.repulse.distance) * (-1 * Math.pow(i / l, 2) + 1) * l * 100, 0, 50),
        ...
        } else if (s.interactivity.events.onclick.enable && isInArray("repulse", s.interactivity.events.onclick.mode)) if (s.tmp.repulse_finish || (s.tmp.repulse_count++, s.tmp.repulse_count == s.particles.array.length && (s.tmp.repulse_finish = !0)), s.tmp.repulse_clicking) {
            var l = Math.pow(s.interactivity.modes.repulse.distance / 6, 3),
                v = s.interactivity.mouse.click_pos_x - e.x, p = s.interactivity.mouse.click_pos_y - e.y,
                d = v * v + p * p, m = -l / d * 1;
            d <= l && function () {
                var a = Math.atan2(p, v);
                if (e.vx = m * Math.cos(a), e.vy = m * Math.sin(a), "bounce" == s.particles.move.out_mode) {
                    var t = e.x + e.vx, i = e.y + e.vy;
                    t + e.radius > s.canvas.w ? e.vx = -e.vx : t - e.radius < 0 && (e.vx = -e.vx), i + e.radius > s.canvas.h ? e.vy = -e.vy : i - e.radius < 0 && (e.vy = -e.vy)
                }
            }()
        ...
        "window" == s.interactivity.detect_on ? s.interactivity.el = window : s.interactivity.el = s.canvas.el, (s.interactivity.events.onhover.enable || s.interactivity.events.onclick.enable) && (s.interactivity.el.addEventListener("mousemove", function (e) {
            if (s.interactivity.el == window) var a = e.clientX,
                t = e.clientY; else a = e.offsetX || e.clientX, t = e.offsetY || e.clientY;
            s.interactivity.mouse.pos_x = a, s.interactivity.mouse.pos_y = t, s.tmp.retina && (s.interactivity.mouse.pos_x *= s.canvas.pxratio, s.interactivity.mouse.pos_y *= s.canvas.pxratio), s.interactivity.status = "mousemove"
        ...
        if (s.particles.number.density.enable) {
            ...
            var a = e * s.particles.number.value / s.particles.number.density.value_area,
                t = s.particles.array.length - a;
            t < 0 ? s.fn.modes.pushParticles(Math.abs(t)) : s.fn.modes.removeParticles(t)
        }
    }, s.fn.vendors.checkOverlap = function (e, a) {
        for (var t = 0; t < s.particles.array.length; t++) {
            var i = s.particles.array[t], n = e.x - i.x, r = e.y - i.y;
            Math.sqrt(n * n + r * r) <= e.radius + i.radius && (e.x = a ? a.x : Math.random() * s.canvas.w, e.y = a ? a.y : Math.random() * s.canvas.h, s.fn.vendors.checkOverlap(e))
        }
    }, s.fn.vendors.createSvgImg = function (e) {
        var a = s.tmp.source_svg.replace(/#([0-9A-F]{3,6})/gi, function (a, t, i, s) {
                ...
                return n
            }), t = new Blob([a], {type: "image/svg+xml;charset=utf-8"}), i = window.URL || window.webkitURL || window,
            n = i.createObjectURL(t), r = new Image;
        r.addEventListener("load", function () {
            e.img.obj = r, e.img.loaded = !0, i.revokeObjectURL(n), s.tmp.count_svg++
        }), r.src = n
    }, s.fn.vendors.destroypJS = function () {
        cancelAnimationFrame(s.fn.drawAnimFrame), t.remove(), pJSDom = null
    }, s.fn.vendors.drawShape = function (e, a, t, i, s, n) {
        ...
        if (s.tmp.img_error = void 0, "" != s.particles.shape.image.src) if ("svg" == e) {
            ...
            a.open("GET", s.particles.shape.image.src), a.onreadystatechange = function (e) {
                4 == a.readyState && (200 == a.status ? (s.tmp.source_svg = e.currentTarget.response, s.fn.vendors.checkBeforeDraw()) : (console.log("Error pJS - Image not found"), s.tmp.img_error = !0))
            }, a.send()
        } else {
            var t = new Image;
            t.addEventListener("load", function () {
                s.tmp.img_obj = t, s.fn.vendors.checkBeforeDraw()
            }), t.src = s.particles.shape.image.src
        } else console.log("Error pJS - No image.src"), s.tmp.img_error = !0
    }, s.fn.vendors.draw = function (e) {
        var a = s.fps_limit;
        e < i + 1e3 / a ? requestAnimFrame(s.fn.vendors.draw) : (delta = e - i, i = e, "image" == s.particles.shape.type ? "svg" == s.tmp.img_type ? s.tmp.count_svg >= s.particles.number.value ? (s.fn.particlesDraw(delta), s.particles.move.enable ? s.fn.drawAnimFrame = requestAnimFrame(s.fn.vendors.draw) : cancelRequestAnimFrame(s.fn.drawAnimFrame)) : s.tmp.img_error || (s.fn.drawAnimFrame = requestAnimFrame(s.fn.vendors.draw)) : void 0 != s.tmp.img_obj ? (s.fn.particlesDraw(delta), s.particles.move.enable ? s.fn.drawAnimFrame = requestAnimFrame(s.fn.vendors.draw) : cancelRequestAnimFrame(s.fn.drawAnimFrame)) : s.tmp.img_error || (s.fn.drawAnimFrame = requestAnimFrame(s.fn.vendors.draw)) : (s.fn.particlesDraw(delta), s.particles.move.enable ? s.fn.drawAnimFrame = requestAnimFrame(s.fn.vendors.draw) : cancelRequestAnimFrame(s.fn.drawAnimFrame)))
    }, s.fn.vendors.checkBeforeDraw = function () {
        "image" == s.particles.shape.type ? "svg" == s.tmp.img_type && void 0 == s.tmp.source_svg ? s.tmp.checkAnimFrame = requestAnimFrame(check) : (cancelRequestAnimFrame(s.tmp.checkAnimFrame), s.tmp.img_error || (s.fn.vendors.init(), s.fn.vendors.draw())) : (s.fn.vendors.init(), s.fn.vendors.draw(0))
    ...
    }, s.fn.vendors.eventsListeners(), s.fn.vendors.start(), document.addEventListener("visibilitychange", function () {
        document.hidden ? (!0, cancelRequestAnimFrame(s.fn.drawAnimFrame)) : (!1, i = performance.now(), s.fn.vendors.draw(0))
    }, !1)
};
